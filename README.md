...
- **概览 (Overview)**：仪表盘展示，包含种田游戏化界面、今日任务、积分进度等核心数据。
- **项目（Project）**：支持 life（生活）| code（代码）两种类型，包含目标设定与标签分类。
- **需求管理（Requirement）**：描述功能需求，包含用户角色、优先级、开始时间、截止时间、子需求拆分。
- **任务管理（Task）**：四种任务类型——爱好 (Hobby)、习惯 (Habit)、任务 (Task)、欲望 (Desire)，支持标题、说明、状态、优先级、预计时长、截止时间等属性。
- **缺陷管理（Defect）**：针对代码项目，记录缺陷标题、严重程度、状态、关联仓库信息。
- **奖励机制（Reward）**：基于积分、徽章、等级、兑换的激励系统，通过种田游戏可视化展示成长进度。
