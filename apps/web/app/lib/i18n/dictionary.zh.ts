export const zhCN = {
  common: {
    buttons: {
      login: '登录',
      register: '注册',
      save: '保存',
      cancel: '取消',
      delete: '删除',
      edit: '编辑',
      confirm: '确认',
    },
    errors: {
      networkError: '网络错误，请稍后重试',
      unauthorized: '未授权，请先登录',
      forbidden: '没有权限执行此操作',
      notFound: '请求的资源不存在',
      serverError: '服务器内部错误',
    },
    loading: '加载中...',
    time: {
      beijing: '北京时间 (UTC+8)',
      tokyo: '东京时间 (UTC+9)',
      newyork: '纽约时间 (UTC-5)',
    },
  },
  login: {
    title: '登录',
    subtitle: '让每天都充实且有意义',
    nickname: '昵称',
    password: '密码',
    confirmPassword: '确认密码',
    loginButton: '登录',
    registerButton: '注册',
    switchingToRegister: '注册',
    switchingToLogin: '登录',
    loading: {
      loggingIn: '登录中...',
      signingUp: '注册中...',
    },
    hints: {
      joinUs: '加入我们，开始你的旅程。',
      makeFulfilling: '让每天都充实且有意义。',
    },
    success: {
      welcomeBack: '欢迎回来，{nickname}！',
      registerSuccess: '注册成功，欢迎 {nickname}！',
    },
    errors: {
      nicknameRequired: '请填写昵称和密码',
      allFieldsRequired: '请填写所有字段',
      passwordMismatch: '两次输入的密码不一致',
      passwordTooShort: '密码长度至少6位',
      loginFailed: '登录失败: {message}',
      registerFailed: '注册失败: {message}',
      requestFailed: '登录请求失败，请稍后重试',
      registerRequestFailed: '注册请求失败，请稍后重试',
    },
    focus: {
      present: '专注当下',
      future: '规划未来',
      achieveBetter: '成就更好的自己。',
    },
    guards: {
      pleaseLogin: '请先登录后再访问',
    },
  },
  dashboard: {
    nav: {
      overview: '概览',
      habits: '习惯',
      dailies: '日常',
      todos: '待办',
      rewards: '奖励',
      project: '项目',
      requirement: '需求',
      task: '任务',
      defect: '缺陷',
      notifications: '通知',
      setting: '设置',
    },
  },
  setting: {
    title: '设置',
    profile: {
      title: '个人资料',
      description: '管理您的个人信息和偏好设置',
      username: '用户名',
      email: '邮箱地址',
      language: '语言',
      languageDescription: '选择界面显示语言',
      timezone: '时区',
    },
    notifications: {
      title: '通知设置',
      description: '管理您的通知偏好',
      emailNotifications: '邮件通知',
      emailNotificationsDescription: '接收重要更新的邮件通知',
      pushNotifications: '推送通知',
      pushNotificationsDescription: '接收浏览器推送通知',
      taskReminders: '任务提醒',
      taskRemindersDescription: '任务截止日期前的提醒',
      dailySummary: '每日总结',
      dailySummaryDescription: '每天发送工作总结邮件',
    },
    appearance: {
      title: '外观设置',
      description: '自定义界面外观',
      theme: '主题',
      themeLight: '浅色主题',
      themeDark: '深色主题',
      themeSystem: '跟随系统',
      compactMode: '紧凑模式',
      compactModeDescription: '使用更紧凑的界面布局',
      showCompleted: '显示已完成项目',
      showCompletedDescription: '在列表中显示已完成的项目',
    },
    security: {
      title: '安全设置',
      description: '管理您的账户安全',
      currentPassword: '当前密码',
      newPassword: '新密码',
      confirmPassword: '确认新密码',
      twoFactor: '双因素认证',
      twoFactorDescription: '启用双因素认证以提高账户安全性',
      updatePassword: '更新密码',
    },
    dataManagement: {
      title: '数据管理',
      description: '导出或删除您的数据',
      export: '导出数据',
      deleteAccount: '删除账户',
    },
  },
  task: {
    title: '任务管理',
    filters: {
      all: '全部',
      todo: '待开始',
      inProgress: '进行中',
      review: '待审核',
      done: '已完成',
    },
    priority: {
      low: '低',
      medium: '中',
      high: '高',
      urgent: '紧急',
    },
    progress: '进度',
  },
  defect: {
    title: '缺陷管理',
    filters: {
      all: '全部',
      open: '待处理',
      inProgress: '处理中',
      resolved: '已解决',
      closed: '已关闭',
    },
    severity: {
      low: '低',
      medium: '中',
      high: '高',
      critical: '严重',
    },
    type: {
      bug: '功能缺陷',
      performance: '性能问题',
      ui: '界面问题',
      security: '安全问题',
      compatibility: '兼容性',
    },
    assignee: '负责人',
    reporter: '报告人',
    created: '创建',
    due: '截止',
    environment: '环境',
    steps: '复现步骤',
  },
  project: {
    title: '项目管理',
    subtitle: '管理和跟踪您的项目进度，协调团队协作',
    empty: '没有找到符合条件的项目',
    progress: '进度',
    loading: '加载中...',
    create: '创建项目',
    creating: '创建中...',
    loadFailed: '加载失败',
    saveFailed: '保存失败',
    createFailed: '创建失败',
    deleteFailed: '删除失败',
    deleteFailedRetry: '删除失败，请稍后重试',
    nameRequired: '项目名称不能为空',
    autoSave: '修改后自动保存',
    createdAt: '创建时间',
    updatedAt: '更新时间',
    type: {
      life: '生活',
      code: '代码',
      sprint: '冲刺项目',
      slow: '长期项目',
      slowBurn: '慢燃项目',
    },
    status: {
      normal: '正常',
      at_risk: '有风险',
      out_of_control: '失控',
    },
    priority: {
      high: '高',
      medium: '中',
      low: '低',
    },
    goals: '目标',
    tags: '标签',
    add: '添加项目',
    edit: '编辑项目',
    delete: '删除项目',
    deleteConfirm: '确定要删除此项目吗？',
    startDate: '开始日期',
    endDate: '结束日期',
    startTime: '开始时间',
    deadline: '截止时间',
    deadlineBeforeStart: '截止时间不能早于开始时间',
    drawer: {
      viewMode: '查看',
      editMode: '编辑',
      switchToView: '切换到查看',
      switchToEdit: '切换到编辑',
      save: '保存',
      createTitle: '新建项目',
      editTitle: '项目详情',
    },
    form: {
      name: '项目名称',
      namePlaceholder: '输入项目名称',
      description: '描述',
      coverImage: '背景图',
      descriptionPlaceholder: '支持 Markdown 格式',
      type: '项目类型',
      priority: '优先级',
      goals: '目标',
      addGoal: '添加目标',
      goalPlaceholder: '输入目标',
      tags: '标签',
      addTag: '添加标签',
      cancel: '取消',
      save: '保存',
    },
    avatar: {
      title: '选择头像',
      placeholder: '选择一个表情作为项目头像',
    },
    coverImage: {
      uploadHint: '点击或拖拽文件到此处上传',
      uploadHintSecondary: '支持单文件或批量上传；严禁上传公司数据或其他禁止文件。',
    },
    filters: {
      all: '全部',
      normal: '正常',
      at_risk: '有风险',
      out_of_control: '失控',
    },
    list: {
      emptyHint: '暂无项目，点击下方按钮新建',
    },
  },
  projectDetail: {
    backToProjectList: '返回项目列表',
    switchProject: '切换项目',
    currentProject: '当前项目',
    projectNotFound: '项目不存在',
    defectOnlyForCode: '仅代码项目有缺陷',
    emptyRequirement: '暂无需求',
    emptyTask: '暂无任务',
    emptyDefect: '暂无缺陷',
    loadFailed: '加载失败',
    folderIconTitle: '返回项目列表',
    dropdown: {
      backToProjectList: '返回项目列表',
      switchProject: '切换项目',
      noProjects: '暂无其他项目',
    },
  },
  requirement: {
    title: '需求管理',
    subtitle: '管理产品需求，跟踪开发进度，确保项目按计划进行',
    empty: '没有找到符合条件的需求',
    add: '添加需求',
    edit: '编辑需求',
    create: '创建需求',
    delete: '删除需求',
    deleteConfirm: '确定要删除这个需求吗？',
    filters: {
      all: '全部',
      draft: '草稿',
      review: '评审中',
      approved: '已批准',
      development: '开发中',
      testing: '测试中',
      completed: '已完成',
    },
    status: {
      draft: '草稿',
      review: '评审中',
      approved: '已批准',
      development: '开发中',
      testing: '测试中',
      completed: '已完成',
      rejected: '已拒绝',
    },
    priority: {
      critical: '紧急',
      high: '高',
      medium: '中',
      low: '低',
    },
    type: {
      feature: '功能',
      enhancement: '增强',
      bugfix: '修复',
      research: '研究',
    },
    view: {
      list: '列表视图',
      kanban: '看板视图',
      groupByStatus: '按状态',
      groupByPriority: '按优先级',
    },
    form: {
      title: '标题',
      description: '描述',
      type: '类型',
      status: '状态',
      priority: '优先级',
      assignee: '分配给',
      reporter: '报告人',
      createdDate: '创建日期',
      dueDate: '截止日期',
      storyPoints: '故事点数',
      tags: '标签',
    },
  },
  rewards: {
    subtitle: '用你赚取的金币购买奖励，激励自己完成任务',
    rewardYourself: '奖励自己',
    buyRewards: '通过完成任务赚取金币来购买奖励！',
  },
  overview: {
    search: 'Search',
    tags: 'Tags',
    addTask: 'Add Task',
    addHabit: 'Add a Habit',
    addDaily: 'Add a Daily',
    addTodo: 'Add a To Do',
    addReward: 'Add a Reward',
    noHabits: 'No habits yet',
    noDailies: 'No dailies yet',
    noTodos: 'No todos yet',
    noRewards: 'No rewards yet',
    filters: {
      all: 'All',
      strong: 'Strong',
      weak: 'Weak',
      due: 'Due',
      notDue: 'Not Due',
      active: 'Active',
      scheduled: 'Scheduled',
      complete: 'Complete',
      custom: 'Custom',
      wishlist: 'Wishlist',
    },
  },
  component: {
    section: {
      add: 'Add a',
    },
  },
  modal: {
    project: {
      createTitle: '创建项目',
      editTitle: '编辑项目',
      name: '项目名称',
      type: '项目类型',
      points: '积分',
      pointsAutoCalculate: '自动计算',
      pointsTooltip: '根据项目优先级自动计算积分值',
      priority: '优先级',
      startTime: '开始时间',
      endTime: '截止时间',
      description: '项目描述',
      status: '项目状态',
      createButton: '创建项目',
      saveButton: '保存',
      cancelButton: '取消',
      // 指标相关
      indicators: '积累指标',
      indicatorsTip: '请在下方设置积累指标',
      indicatorsTotalWeight: '总权重',
      indicatorsProgress: '当前进度',
      addIndicator: '添加指标',
      indicatorName: '指标名称',
      indicatorValue: '当前值',
      indicatorTarget: '目标值',
      indicatorWeight: '权重',
    },
  },
  confirm: {
    unsavedChanges: {
      title: '确认关闭',
      message: '确定放弃添加的内容吗？',
      confirm: '确定',
      cancel: '取消',
    },
  },
  validation: {
    project: {
      nameRequired: '请输入项目名称',
      nameMaxLength: '名称不能超过 200 个字符',
      typeRequired: '请选择项目类型',
      startDateRequired: '请选择开始时间',
      pointsRequired: '请输入积分值',
      pointsPositive: '积分必须为正整数',
      timeInvalid: '截止时间不能早于开始时间',
      indicatorRequired: '慢燃项目至少需要设置一个积累指标',
      indicatorNameRequired: '请输入指标名称',
      indicatorNameMaxLength: '指标名称不能超过 50 个字符',
      indicatorTargetRequired: '目标值必须大于 0',
      indicatorWeightRange: '权重需在 0-100 之间',
      indicatorWeightSum: '所有指标权重之和必须等于 100%，当前：{total}%',
    },
  },
  help: {
    project: {
      typeSprint: '短期、高强度、快速迭代的项目',
      typeSlow: '长期、持续、稳步推进的项目',
    },
  },
};
