...
系统 SHALL 在项目详情页顶部展示项目上下文栏，包含文件夹图标、面包屑、项目名、下拉菜单及需求/任务/缺陷/迭代 Tab。

#### Scenario: 顶部栏布局
- **WHEN** 用户进入项目详情页
- **THEN** 顶部栏自左至右显示：文件夹图标、右箭头、项目名、下拉箭头、竖线分隔、需求 Tab、任务 Tab、缺陷 Tab、迭代 Tab
- **AND** 默认选中「需求」Tab
...
#### Scenario: Tab 切换
- **WHEN** 用户点击「需求」「任务」「缺陷」或「迭代」Tab
- **THEN** 切换 Tab 高亮
- **AND** 展示对应内容区
- **AND** URL 可带 query 如 `?tab=requirement|task|defect|iteration`

---

### Requirement: 项目详情页 Tab 内容
系统 SHALL 在项目详情页的需求、任务、缺陷、迭代 Tab 中分别展示当前项目下的对应内容；需求/任务/缺陷按 projectId 筛选，迭代按 projectId 筛选并展示迭代列表。
...
#### Scenario: 迭代 Tab 内容
- **WHEN** 用户选中迭代 Tab
- **THEN** 请求 `GET /api/iterations?projectId=[projectId]`
- **AND** 展示该项目的迭代列表
- **AND** 支持在迭代 Tab 内创建、编辑、删除迭代
